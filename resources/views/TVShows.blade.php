<div class="w-full h-auto min-h-screen flex flex-col bg-zinc-900">
    ...
    <div class="w-auto pl-28 pr-10 pt-6 pb-10 grid grid-cols-3 lg:grid-cols-5 gap-5" id="dataWrapper">
        @foreach ($tvshows as $tvshowsItems)

        @php
        $original_date = $tvshowsItems->first_air_date;
        $timestamp = strtotime($original_date);

        $TVShowsYear = date("Y", $timestamp);
        $TVShowsID = $tvshowsItems->id;
        $TVShowsTitle = $tvshowsItems->name;
        $TVShowsRating = $tvshowsItems->vote_average * 10;
        $TVShowsImage = "{$imageBaseURL}/w500{$tvshowsItems->poster_path}";
...
<script>
    ...
    function loadMore(){
        ...
        .done(function (response){
            ...
            //Mengambil dataset TVShows
            if(response.results){
                var TVShowsDatas = [];

                response.results.forEach(items => {
                    let original_date = items.first_air_date;
                    let date = new Date(original_date);
                    let TVShowsYear = date.getFullYear();
                    let TVShowsID = items.id;
                    let TVShowsTitle = items.name;
                    let TVShowsImage = `${imageBaseURL}/w500${items.poster_path}`;
                    let TVShowsRating = items.vote_average * 10;

                    TVShowsDatas.push(`<a href="tv/${TVShowsID}" class="group">
                            <div class="min-w-[232px] min-h-[428px] bg-zinc-900 drop-shadow-[0_0px_8px_rgba(255,255,255,0.2)] group-hover:drop-shadow-[0_0px_8px_rgba(255,255,255,0.5)] rounded-[32px] p-5 flex flex-col mr-8 duration-100">
                                <div class="overflow-hidden rounded-[32px]">
                                    <img class="w-[full] h-[300px] rounded-[32px] group-hover:scale-105 duration-200" src="${TVShowsImage}">
                                </div>
                ...
                $("#dataWrapper").append(TVShowsDatas.join(""));
            }
        })
