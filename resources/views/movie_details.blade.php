...
<html lang="en">
    ...
    <body>
        <div class="w-full h-screen flex flex-col relative">
            ...
            {{-- Header Section --}}
            <div class="w-full bg-transparent h-[96px] drop-shadow-[0_10px_8px_rgb(255,255,255,0.25)] flex flex-row items-center z-10">
            ...
            @php
                ...
                if($movieData){
                    ...
                    if($movieData->runtime){
                        $hour = (int)($movieData->runtime / 60); //Nyari hour
                        $minute = $movieData->runtime % 60; //Nyari minute
                ...
                if (isset($movieData->videos->results)) {
                    foreach($movieData->videos->results as $items){
                        if(strtolower($items->type) == 'trailer'){
                            $trailerID = $items->key;
                            break;
                        }
                    }
            ...
            <div class="w-full h-full z-10 flex flex-col justify-center px-20">
                ...
                <span class="font-inter text-2xl mt-4 text-white italic line-clamp-5">{{ $tagline }}</span>

                <div class="flex flex-row mt-4 items-center">
                    {{-- Rating Content --}}
                    <div class="w-20 h-20 rounded-full flex items-center justify-center mr-4 bg-cyan-950">
                ...
                {{-- Trailer Button --}}
                @if ($trailerID)
                <button class="w-fit bg-sky-500 text-white pl-4 pr-6 py-3 mt-5 font-inter text-xl flex flex-row rounded-lg items-center hover:drop-shadow-lg duration-200" onclick="showTrailer(true)">
            ...
            <div id="trailerWrapper" class="absolute z-10 w-full h-screen p-20 bg-black flex flex-col">
                <button class="ml-auto group mb-4" onclick="showTrailer(false)">
                    <svg xmlns="http://www.w3.org/2000/svg" height="48" width="48">
                      <path d="m12.45 37.65-2.1-2.1L21.9 24 10.35 12.45l2.1-2.1L24 21.9l11.55-11.55 2.1 2.1L26.1 24l11.55 11.55-2.1 2.1L24 26.1Z" class="fill-white group-hover:fill-sky-500 duration-200"></path>
                    </svg>
                </button>
